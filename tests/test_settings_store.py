...
class TestSettingsStoreLoad:
    def test_returns_defaults_when_no_file_exists(self, isolated_settings):
        ...
        assert result["diff_baseline_mode"] == "snapshot"
        assert result["diff_baseline_path"] == ""
    ...
    def test_legacy_staff_diff_path_migrates_to_new_diff_baseline_keys(self, isolated_settings):
        settings_path = isolated_settings
        settings_path.parent.mkdir(parents=True, exist_ok=True)
        settings_path.write_text(
            json.dumps({"staff_diff_activity_log_path": "/a/ASM-ActivityLog.csv"}),
            encoding="utf-8",
        )

        result = SettingsStore.load()

        assert result["diff_baseline_mode"] == "activity_log"
        assert result["diff_baseline_path"] == "/a/ASM-ActivityLog.csv"
...
class TestSettingsStoreSave:
    def test_round_trip(self, isolated_settings):
        data = {
            ...
            "diff_baseline_mode": "activity_log",
            "diff_baseline_path": "/a/ASM-ActivityLog.csv",
        }
        ...
        assert loaded["diff_baseline_mode"] == "activity_log"
        assert loaded["diff_baseline_path"] == "/a/ASM-ActivityLog.csv"
    ...
    def test_utf8_encoding_preserved(self, isolated_settings):
        ...
        data["diff_baseline_mode"] = "snapshot"
