...
def _roster_key(row: dict) -> str:
    return f"{row['class_id']}:{row['student_id']}"


def _rosters_equal(curr: dict, snap: dict) -> bool:
    """Roster equality excludes roster_id — enrollment identity is class+student."""
    for key in curr.keys() | snap.keys():
        if key == "roster_id":
            continue
        if curr.get(key, "") != snap.get(key, ""):
            return False
    return True
