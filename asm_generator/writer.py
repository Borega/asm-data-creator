...
from .transform import clean_name_part, make_person_id_parts
...
def _sanitize_staff_rows(rows: list[dict]) -> list[dict]:
    ...
    def _fallback_pid(row: dict) -> str:
        ...
        if pn:
            return f"staff.{pn}"
        # No identifying fields: hash the row so the fallback ID is stable across runs.
        token = "|".join(
            f"{k}={(row.get(k, '') or '').strip()}"
            for k in sorted(row.keys())
        ).encode("utf-8")
        digest = hashlib.sha1(token).hexdigest()[:12]
        return f"staff.unknown-{digest}"

    merged: dict[str, dict] = {}
    for raw in rows:
        row = dict(raw)
        pid = (row.get("person_id", "") or "").strip() or _fallback_pid(row)
        row["person_id"] = pid
